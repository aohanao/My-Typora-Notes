...
### 一、merge_dicts 这个 reducer 到底是什么
...
### 三、多智能体是否共用一套 state？分两层说明

#### 1. 顶层主图：所有智能体共用同一个 CAEAgentState

- 整个工作流只有一份全局 state，所有智能体节点读取、修改的都是这一份数据；
- `consensus_params` 存在这份全局 state 里，天然所有智能体共享同一套参数；
- messages、上下文水位、确认标记、脚本路径全全局互通，一个智能体改了，其他智能体立刻能读到。

#### 2. 仿真子图 SimPipelineState：是「临时拷贝」，不是全局共用

子图调用时会**复制一份主 state** 作为子图局部状态：

1. 子图内新增私有字段（重试次数、代码错误、临时提取参数）只会存在这份副本，不会污染全局主状态；
2. 子图跑完后，需要手动指定要同步回主图的字段（生成代码、文件路径等）；
3. 子图内部修改 consensus_params，依靠 reducer 合并，同步逻辑和主图一致。
...
Checkpointer 的核心抽象类是 `langgraph.checkpoint.base.BaseCheckpointSaver`，定义了保存和加载检查点的标准接口。主要方法包括：
...
![image-20260624201057008](./assets/image-20260624201057008.png)
