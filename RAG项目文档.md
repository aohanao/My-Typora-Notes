...
#### 1.**什么是 BGE-Reranker 交叉编码器 (Cross-Encoder)？**
...
#### 2.**RRF (Reciprocal Rank Fusion) 倒数排名融合怎么用？**
...
“你们用大模型做裁判时，**遇到过最大的 Bad Case（误判）是什么**？你们是怎么修改 Prompt 让裁判变聪明的？” “你们跑完评估后，发现 Context Precision（精度）和 Context Recall（召回率）分别是多少？**针对这个较低的数据，你具体改了代码里的哪行配置？** 效果提升了多少？” 
