...
```
[用户上传 PDF] ➔ React 前端（web/src/App.jsx）+ FastAPI 后端（app.py）
      │
      ▼
...
[数据入库] ➔ knowledge_base.py（upload_by_str）
      ...
      └── 3. 向量持久化：Embedding 落盘 ChromaDB + 构建 BM25 稀疏索引
```
...
> JPEG 文件里装的是**压缩后的像素数据和头部元数据**，不是可读字符 —— 它天然是 "二进制" 的。JSON 是**纯文本格式**，语法上只允许放 Unicode 字符、数字、布尔等，**必须先把字节流 "翻译" 成纯 ASCII 文本**—— 这就是 base64 编码做的事，也是你说的 "标准输入形式"，准确术语叫 **base64 编码**。
...
> **MD5 就是 "哈希" 的一种具体实现**。哈希（hash）是统称：把任意长度的数据，通过算法映射成固定长度的 "摘要"。MD5 的摘要固定是 **128 位（16 字节）**，显示为 32 个十六进制字符。
...
> ## 3. 词典里 `100 nz` / `100 n` 的含义
>
> `cae_dict.txt` 每行是 jieba 用户词典的标准三字段格式：**词 词频 词性**
>
> ```
> C30喷射混凝土 100 nz
> 弹性模量      100 n
> ```
>
> - **数字和字母**：是因为这些词本身就是 "中英数字混合" 的术语（`HPB300钢`、`C30喷射混凝土`、`V级围岩`、`FLAC3D`）。纯字母数字串（如 FLAC3D）jieba 本来就会当一个 token，加词典主要是让**混合词整词化**并提高权重。
> - **`100` = 词频（freq）**：决定分词时这个词的 "先验概率" 权重。jieba 用动态规划找最大概率路径，词频越高越倾向把它切成完整词而不是拆碎。想压制某个词的拆分，把词频调大即可。
> - **`n` / `nz` = 词性标注**：`n` 普通名词，`nz` 专有名词（软件名、材料牌号这类）。**注意：词性不影响切词结果**（切词只依赖词频），它只会在 `jieba.posseg` 词性标注时被用到，属于规范标注。
>
> **结巴分词的原理**
>
> jieba 是 "词典 + 统计" 混合方案，核心分三步：
>
> **① 词典 + DAG 构建（把所有可能的切法列出来）**
>
> jieba 内置一个约 35 万词的词典（`dict.txt`），每个词带词频。对一句话，它先建一棵**前缀树**（trie），然后为句子的每个位置生成一张 **DAG（有向无环图）**：记录 "从这个字开始，往后能组成哪些词典里的词"。
>
> 例：`我 爱 北 京 天 安 门`，位置 0 的 "我" 能匹配 `我`；位置 2 的 "北" 能匹配 `北`、`北京`（如果词典里还有 "北京欢迎你" 也会列出来）。
>
> **② 动态规划找最大概率路径（精确模式的核心）**
>
> 有了所有候选切法，jieba 用**动态规划**选一条 "整句话概率最大" 的切分路径：每个词有词频（近似看作概率），把所有候选词按顺序拼起来，找乘积最大（实际取 log 后求和最大）的那条路径。
>
> ```
> P(我) × P(爱) × P(北京) × P(天安门)  →  最大化这条乘积
> ```
>
> 这就是 "精确模式"（`jieba.cut` 默认）：追求整句最优，宁可少切也不切碎。比如 `我爱北京天安门` 在精确模式下是 `我 / 爱 / 北京 / 天安门`，而不会切成 `我 / 爱 / 北京 / 天 / 安 / 门`（因为 "天安门" 词频高）。
>
> **③ HMM 处理 "词典里没有的词"（未登录词）**
>
> 句子总会出现词典覆盖不到的新词、人名、网络词。jieba 对这些位置启动一个**隐马尔可夫模型（HMM）**：
>
> - 每个字标注一个状态：**B**（词首）/ **M**（词中）/ **E**（词尾）/ **S**（单字成词）；
> - 模型有两组概率：字与字之间的**状态转移概率**（比如 B 后面多半是 M 或 E）、每个字出现在某状态下的**发射概率**（比如 "的" 更常是 E 或 S）；
> - 用 **Viterbi 算法**解码出整句话最可能的状态序列，再把连续的 B/M/E 合并成词。
>
> **④ 你项目用的 `cut_for_search`（搜索引擎模式）**
>
> 在精确模式基础上，**把长度超过 2 个字的词再按词典二次切碎**，提高召回率。比如 `天安门` 会额外给出 `天安`、`安门` 这种子词。这对 BM25 很有意义 —— 关键词检索靠的是 "命中即得分"，词切得越碎，用户问 "安门构造" 时越可能命中。代价是分词结果变多、有歧义，但检索场景要的就是召回优先。
...
![image-20260910112252050](Agentic_RAG项目文档.assets/image-20260910112252050.png)
...
> Streamlit 是一个专为数据科学家和机器学习工程师设计的 Python 框架，其核心优势在于能用极少的代码快速将 Python 脚本转化为交互式 Web 应用。Streamlit的脚本执行模型是**同步且单线程**的。
>
> 📊 **Streamlit 优缺点一览**
>
> | 维度           | ✅ 优点                                                       | ❌ 缺点                                                       |
> | :------------- | :----------------------------------------------------------- | :----------------------------------------------------------- |
> | **开发体验**   | **极速开发**：纯 Python 脚本式写法，无需前端知识，几分钟即可搭建原型 | **代码易混乱**：对于复杂应用，缺乏强制的结构规范，代码可能变得难以维护 |
> | **功能与性能** | **数据集成简单**：与 Pandas、NumPy、Matplotlib 等数据科学库无缝集成，内置丰富图表组件 | **页面“全量刷新”**：任何交互都会重新运行整个脚本，处理复杂工作流或大数据时效率低 |
> | **用户界面**   | **默认颜值高**：内置简洁美观的现代化主题，无需设计即可获得专业外观 | **布局控制有限**：控件位置和样式相对固定，难以实现高度定制化的复杂交互界面 |
> | **部署与分享** | **一键部署**：官方 Streamlit Community Cloud 与 GitHub 集成，可免费托管公开应用 | **生产环境挑战**：企业级部署方案（如身份验证、安全、扩展性）仍需结合云服务自行搭建 |
...
![image-20260910115100180](Agentic_RAG项目文档.assets/image-20260910115100180.png)
...
> 是一种基于**概率检索模型**的经典关键词匹配打分算法，属于 TF-IDF 算法家族的演进版本。在现代检索增强生成（RAG）和混合检索架构中，BM25 凭借对**词频饱和度**和**文档长度归一化**的优化，成为稀疏检索的核心组件。

> ### 一句话先搞懂：它们在干嘛？
>
> 不管是 **TF-IDF** 还是 **BM25**，它们的目的只有一个：**给文章打分，判断这篇文章和你想搜的词有多相关。**
>
> ### 1. 老前辈：TF-IDF 是怎么想的？
>
> 它主要看两件事：
>
> 1. **词频（TF）：** 你搜的词在这篇文章里出现的次数越多，说明文章越相关（比如搜“挖掘机”，文章里提了 10 次，得分就高）。
> 2. **逆文档频率（IDF）：** 这个词是不是“大路货”？
>    - 像“的”、“在”、“是”这种到处都有的词，不值钱，得分极低。
>    - 像“地质沉降”、“岩爆”这种很少见但在你文章里出现的专业词，非常值钱，得分极高。
>
> **但它有个致命毛病（死脑筋）：**
>
> - **容易被“刷词”骗：** 只要一篇文章故意把“挖掘机”复制粘贴 1000 遍，TF-IDF 就会觉得这篇文章好上天了。
> - **对长文章不公平：** 一篇 1 万字的长文章，可能刚好顺带提了几次关键词，得分就会莫名其妙比一篇 200 字但字字珠玑的短文章还高。
>
> ### 2. 新主力：BM25 做了什么改进？
>
> BM25 就是给 TF-IDF 加了**两个“防作弊”和“讲道理”的开关**：
>
> #### ① 边际效应递减（防止刷词）
>
> - 词出现 1 次到 3 次，得分**蹭蹭涨**；
> - 出现 10 次到 20 次，得分**涨得慢了**；
> - 出现 100 次和 1000 次，得分**基本封顶，几乎没区别**。
>
> > **通俗理解：** 喝第 1 杯水救命，喝第 3 杯解渴，喝第 100 杯不但没用还会撑死。BM25 绝不给无脑堆砌关键词的人多加分。
>
> #### ② 考虑文章长短（公平竞争）
>
> - 如果一篇**短文章**（比如 100 字）提了 3 次关键词，BM25 会觉得：“含金量极高，加分！”
> - 如果是一本**厚书**（比如 10 万字）只提了 3 次关键词，BM25 会觉得：“你大概率只是顺嘴提了一句，扣点分。”
...
> **做法：直接数数。**
>
> 假设有一篇文档，分完词之后总共有 100 个词：
>
> - 如果“隧道”出现了 5 次，那么它的绝对词频 $f = 5$。
>
> - 在传统 TF-IDF 里，为了消除文档总字数的影响，通常会算一个**相对词频**：
>
>   $$\text{TF} = \frac{\text{该词在文档中出现的次数}}{\text{文档总词数}} = \frac{5}{100} = 0.05$$
>
> - 统计方式非常朴素：就是建一个计数器（字典/哈希表），文档分词后遍历一遍，统计每个词的频次。
>
> ### 二、 IDF（逆文档频率）是怎么统计的？
>
> **做法：看“包含这个词的文档”占“总文档库”的比例。**
>
> 假设你的知识库里一共有 **$N = 10,000$ 篇**文档：
>
> 1. **情况 A（高频废话词）：** 比如“的”或者“系统”，在 $9,900$ 篇文档里都出现过。
>    - 占比是 $\frac{9900}{10000} = 99\%$，说明这个词根本没有区分度。
>    - 它的 IDF 计算结果就会**极小，甚至接近 0**。
> 2. **情况 B（罕见专业词）：** 比如“全断面硬岩掘进机”，整套库里只有 **$5$ 篇**文档提到过。
>    - 占比是 $\frac{5}{10000} = 0.05\%$，极其稀缺。
>    - 它的 IDF 计算结果就会**非常大**。
>
> > **计算小细节：**
> >
> > 为什么公式里总喜欢套一个“对数（$\log$ 或 $\ln$）”？
> >
> > 如果库里有 100 万篇文档，某个词只有 1 篇有，比例是 100 万倍；如果不取对数，这个词的分数会大到把其他所有词完全碾压。取了对数，就能把成千上万倍的差距压缩到一个合理的温和区间（比如把 100 万压缩成 13 左右）。
>
> ### 三、 BM25 的“边际效应递减”怎么算出来的？
>
> BM25 抛弃了传统 TF 的简单相乘，设计了一个**类似“天花板”的分式**：
>
> $$\text{词频得分} = \frac{\text{TF}}{\text{TF} + k_1}$$
>
> 这里的 $k_1$ 是一个调节参数（通常取值在 $1.2 \sim 2.0$ 之间，比如我们设 $k_1 = 1.5$）。
>
> 我们来看看随着词频 $\text{TF}$ 增加，分数值是怎么变化的：
>
> - 当 $\text{TF} = 1$ 时：$\frac{1}{1 + 1.5} = \mathbf{0.40}$（第一次出现，贡献了 0.4 分）
> - 当 $\text{TF} = 2$ 时：$\frac{2}{2 + 1.5} = \mathbf{0.57}$（增加了 0.17 分）
> - 当 $\text{TF} = 5$ 时：$\frac{5}{5 + 1.5} = \mathbf{0.77}$（涨幅开始变平缓）
> - 当 $\text{TF} = 20$ 时：$\frac{20}{20 + 1.5} = \mathbf{0.93}$
> - 当 $\text{TF} = 1000$ 时：$\frac{1000}{1000 + 1.5} \approx \mathbf{0.998}$
>
> 无论你怎么狂刷词频，整个分数值永远超不过 **$1.0$**（它的数学极限）。这个分母里的 `+ k1` 就起到了“刹车”的作用。
>
> ### 四、 BM25 的“文章长短”是如何评判的？
>
> BM25 评判长短的标准不是看“固定多少字算长”，而是看**相对比例**。
>
> 1. **第一步：算出整个库的平均长度（$\text{avgdl}$）**
>
>    - 把知识库里所有切片/文章的总字数加起来，除以总篇数。假设平均每篇切片是 **300 字**。
>
> 2. **第二步：计算当前文档的“相对长短倍数”**
>
>    - $\text{相对长度} = \frac{\text{当前文档长度}}{\text{平均长度 (300)}}$
>    - 如果当前文档有 600 字，相对长度就是 $2.0$（比平均长一倍）；
>    - 如果当前文档只有 150 字，相对长度就是 $0.5$（只有平均的一半）。
>
> 3. **第三步：把这个倍数塞进分母里当“惩罚/奖励因子”**
>
>    - BM25 引入了一个参数 $b$（通常取 $0.75$），用来决定你对文章长度有多在乎：
>
>      $$\text{调整项} = 1 - b + b \cdot \left(\frac{\text{当前文档长度}}{\text{平均长度}}\right)$$
>
>    - **如果文章很长（如 600 字）：** 调整项变大 $\rightarrow$ 整个分母变大 $\rightarrow$ **最终得分被除小了（长文惩罚）**。
>
>    - **如果文章很短（如 150 字）：** 调整项变小 $\rightarrow$ 整个分母变小 $\rightarrow$ **最终得分被放大了（短文奖励）**。
